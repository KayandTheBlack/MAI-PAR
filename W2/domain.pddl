(define (domain ghostandrobot)
  ...
  (:predicates
    ...
    (at ?e - entity ?i - gridindex ?j - gridindex) ; Entity e is at coordinates i, j
    ; The two following predicates tell whether I can move the entities BEFORE advancing the time.
    (movedghost) 
    (movedrobot)
  ...
  ;   Advance time resets the moved predicates, whilst advancing the time. Requires the robot to have 'moved' so as to make it pay for the position it is in (via the action stay, for example)
  ;   Move actions (up down left right and stay) move the robot, make it pay the cost of the position, and keep it from moving until time advances once again.
  ;   Move Ghost. This action is different from the one above, as the ghost's movements are restricted to the time sequence given by the problem specification.
  ;   Finish. This action sets the end predicate true, and requires the bot and ghost to be in the same place.
